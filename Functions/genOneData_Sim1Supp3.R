...
#' 
#' `genOneData_Sim1Supp3()` generates clustered data consisting of a level-1 treatment, 
...
#' @examples
#' genOneData_Sim1Supp3(num_clust = 30, clust_size = 30)
#' 
genOneData_Sim1Supp3 <-
  function(num_clust,
           clust_size,
           num_x = 3,
           iccx = 0.2,
    ...
    # z
    a_z <- 0.476 # on trt R^2=0.05005643  #(a_z^2) / ((a_x^2)*num_x + a_z^2 + (pi^2/3)/4 + (pi^2/3)) 
    b_z <- 0.55 # on med R^2=0.04996915  #(b_z^2) / ((b_x^2)*num_x + b_z^2 + treat_m^2 + (pi^2/3)/4 + (pi^2/3)) 
    c_z <- 0.638 # on outcome R^2=0.04994785  #(c_z^2) / (c_z^2 + treat_y^2 + (c_x^2)*num_x + med_y^2 + (pi^2/3)/4 + (pi^2/3)) 
    # effects 
