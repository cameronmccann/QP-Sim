...
#'
#' @example
#' # Example usage of bootstrap_ci_re_mean_paral
#' result_re <- bootstrap_ci_re_mean_paral(iterations = 10,
#'                                     iptw = iptw_re, 
#'                                     data = data, 
#'                                     cores = 6, 
...
bootstrap_ci_re_mean_paral <- function(iterations = 50, iptw, data, cores = 2, core_seeds = NULL, effect_type = "PNIE") {
  ...
  bootstrap_iteration <- function(i, core_seed, iptw_str, data, effect_type) {
    ...
    # Add these means back to the original dataset
    ...
    outcome_formula <- if (effect_type == "TNIE") {
      depress_w4 ~ selfEst_w3_sc * sportPartic_w1 + cluster_mean_sportPartic_w1 + cluster_mean_selfEst_w3_sc + age_w1_sc + sex_w1 + white_w1 + black_w1 + 
        parentalEdu_w1_sc + familyStruct_w1 + (1 | CLUSTER2)
    } else {
      depress_w4 ~ selfEst_w3_sc + sportPartic_w1 + cluster_mean_sportPartic_w1 + cluster_mean_selfEst_w3_sc + age_w1_sc + sex_w1 + white_w1 + black_w1 + 
        parentalEdu_w1_sc + familyStruct_w1 + (1 | CLUSTER2)
