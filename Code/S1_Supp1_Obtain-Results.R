...
relBias_PNIE_Table70 <- relBias_Tbl70 %>%
  ...
  arrange(`Mediator/Outcome Model`, Med)
...
gglayer_labs_Supp1 <- list(
  ...
  guides(y.sec = guide_none("Outcome Model"),
         x.sec = guide_none("Cluster Size"))
)
...
readRDS(file = paste0(path, "/Data/S1_Supp1_Simulation-Data.rds")) |> 
  ...
  # visual 
  # ggplot(aes(x = abs((PNIE_est / PNIE) - 1), y = interaction(medmodel, outModel, sep = " - "), fill = `PS Model`)) +
  ggplot(aes(x = abs((PNIE_est / PNIE) - 1), y = medmodel, fill = `PS Model`)) +
  geom_vline(xintercept = 0, linewidth = 0.5) +
  geom_vline(xintercept = 0.10, color = "red", alpha = 0.8, linewidth = 0.7) +
  geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.8, 
               linewidth = 0.3, 
                             ...
                             ifelse(PS == "RE", "Random-Effect", 
                                    ifelse(PS == "SL", "Single-Level", 
                                           ifelse(PS == "RE-Mean", "Random-Effect Mean", 
                                                  "ERROR"))))) %>% 
  # visual 
  ggplot(aes(x = (PNIE_est / PNIE) - 1, y = medmodel, fill = `PS Model`)) +
  # ggplot(aes(x = abs((PNIE_est / PNIE) - 1), y = medmodel, fill = `PS Model`)) +
  geom_vline(xintercept = 0, linewidth = 0.5) +
  geom_vline(xintercept = 0.10, color = "red", alpha = 0.8, linewidth = 0.7) +
  geom_vline(xintercept = -0.10, color = "red", alpha = 0.8, linewidth = 0.7) +
...
# Save visual 
ggsave(filename = paste0(path, "/Figures/PNIE-Relative-Bias-Boxplot_num_clust-70_icc-20.png"), plot = last_plot())
...
# Save visual 
ggsave(filename = paste0(path, "/Figures/PNIE-Relative-Bias-Boxplot_num_clust-100_icc-20.png"), plot = last_plot())
...
gglayer_labs_Supp1 <- list(
  ...
  guides(y.sec = guide_none("Outcome Model"),
         x.sec = guide_none("Mediator Model"))
)
...
# Save plot
ggsave(filename = paste0(path, "/Figures/",
                         "PNIE-Coverage-Lineplot_num_clust-100_icc-20.png"),
       plot = last_plot())
