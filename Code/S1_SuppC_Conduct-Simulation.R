...
source("Functions/genOneData_Sim1SuppC.R") # has weaker confounding 
...
reps <- 1#000 #200 #                      # Total number of replications per condition
dir.create(path = "Output/S1_SuppC_Simulation-Output")
dir.create(path = "Output/S1_SuppC_Simulation-Output/2025-03-02_1000-reps", showWarnings = FALSE)
dir.create(path = "Output/S1_SuppC_Simulation-Output/2025-03-02_1000-reps/interim", showWarnings = FALSE)
path <- "Output/S1_SuppC_Simulation-Output/2025-03-02_1000-reps"
...
for (condition in 1:nrow(cond)) {
  ...
  for (block in seq(1, reps, by = 100)) {
    ...
    block_time <- system.time({
      block_results <- foreach::foreach(
        i = block:block_end,
        .combine = rbind,
        .export = c("genOneData_Sim1SuppC", "AnalysisFunc_Sim1c") #"AnalysisFunc_Sim1b")
      ) %dopar% {
    ...
    file_name <- paste0(
      path, "/interim/", "/S1_SuppC_Condition-", condition,
  ...
  overall_file_name <- paste0(
    path, "/S1_SuppC_Condition-", condition,
...
output_file <- paste0(path, "/S1_SuppC_Computation-Time.rds")
